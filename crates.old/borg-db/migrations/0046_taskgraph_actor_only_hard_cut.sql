-- Actor-only TaskGraph hard cut.
-- No backfill: drop legacy session-oriented TaskGraph tables and recreate.

DROP TABLE IF EXISTS taskgraph_task_labels;
DROP TABLE IF EXISTS taskgraph_task_blocked_by;
DROP TABLE IF EXISTS taskgraph_task_references;
DROP TABLE IF EXISTS taskgraph_comments;
DROP TABLE IF EXISTS taskgraph_events;
DROP TABLE IF EXISTS taskgraph_tasks;
...
CREATE INDEX IF NOT EXISTS idx_taskgraph_tasks_assignee_actor_status ON taskgraph_tasks(assignee_actor_id, status);
CREATE INDEX IF NOT EXISTS idx_taskgraph_tasks_reviewer_actor_status ON taskgraph_tasks(reviewer_actor_id, status);

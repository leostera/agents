...
fn render_manifest(_workspace_root: &Path, crates: &[EvalCrate]) -> String {
    ...
    let crates_dir = crate_dir
        ...
        .to_path_buf();
    let agents_dir = crates_dir.join("agents");
    let evals_dir = crate_dir.to_path_buf();
...
fn render_main(crates: &[EvalCrate]) -> Result<String> {
    ...
    let file: syn::File = parse_quote! {
        ...
        async fn main() -> Result<()> {
            ...
            let workspace_root = ::evals::runner::resolve_workspace_root(&std::env::current_dir()?)?;
            let loaded = ::evals::runner::load_workspace_run_config(&workspace_root)?;
            let run_config = loaded.run_config;
            let output_dir = loaded.output_dir;

            match command.as_str() {
                "list" => {
                    ::evals::runner::list_discovered(&registries, &run_config, json);
                }
                "run" => {
                    ::evals::runner::run_discovered(
                        registries,
                        run_config,
                        &output_dir,
                        ::evals::runner::RunOptions {
                            json,
                            filter: ::evals::TargetFilter {
                                query,
                                model,
                            },
                        },

...
impl LlmProvider for LmStudio {
    ...
    async fn chat_raw(&self, req: RawCompletionRequest) -> LlmResult<RawCompletionResponse> {
        ...
        let messages: Vec<crate::llm::provider::lm_studio::ChatMessage> = req
            .input
            .iter()
            .map(|item| crate::llm::provider::lm_studio::ChatMessage {
                role: match item {
        ...
        let chat_req = crate::llm::provider::lm_studio::ChatRequest {
            model: model.clone(),
            messages,

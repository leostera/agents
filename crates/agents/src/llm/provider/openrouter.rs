...
use crate::llm::error::{Error, LlmResult, OpenRouterConfigError};
...
impl LlmProvider for OpenRouter {
    ...
    async fn chat_raw(&self, req: RawCompletionRequest) -> LlmResult<RawCompletionResponse> {
        ...
        let messages: Vec<crate::llm::provider::openrouter::ChatMessage> = req
            .input
            .iter()
            .map(|item| crate::llm::provider::openrouter::ChatMessage {
                role: match item {
        ...
        let chat_req = crate::llm::provider::openrouter::ChatRequest {
            model: model.clone(),
            messages,

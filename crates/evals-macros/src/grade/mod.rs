...
fn expand_grade(args: &GradeArgs, input: &ItemFn) -> Result<TokenStream> {
    ...
    let wrapper_fn: ItemFn = parse_quote! {
        pub fn #wrapper_ident() -> ::evals::Grader<#state_ty, #output_ty> {
            ::evals::predicate(#grade_name, |trial, ctx| async move {
...
mod tests {
    ...
    fn expands_grade_wrapper_snapshot() {
        ...
        pub fn echoes_empty() -> ::evals::Grader<EchoHarness, EchoRes> {
            ::evals::predicate(
                "echoes-empty",

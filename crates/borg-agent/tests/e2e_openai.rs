...
use borg_llm::testing::{optional_test_env, runner_with_openai_model};
...
#[tokio::test]
async fn openai_agent_send_completes_text_turn_long() -> LlmResult<()> {
...
#[tokio::test]
async fn openai_agent_run_streams_text_turn_long() -> LlmResult<()> {
    let runner = runner_with_openai_model(&openai_model())?;
...
#[tokio::test]
async fn openai_agent_send_decodes_typed_response_long() -> LlmResult<()> {
    let runner = runner_with_openai_model(&openai_model())?;
    let mut agent = Agent::builder()
...
#[tokio::test]
async fn openai_agent_executes_ping_tool_and_finishes_long() -> LlmResult<()> {
    let runner = runner_with_openai_model(&openai_model())?;
...
#[tokio::test]
async fn openai_agent_run_executes_ping_tool_and_finishes_long() -> LlmResult<()> {
    let runner = runner_with_openai_model(&openai_model())?;
    let tool_runner = CallbackToolRunner::new(|call: ToolCallEnvelope<TestTools>| async move {
        match call.call {
            TestTools::Ping { value } => Ok(ToolResultEnvelope {
...
#[tokio::test]
async fn openai_agent_cancel_during_active_turn_long() -> LlmResult<()> {
    let runner = runner_with_openai_model(&openai_model())?;
    let tool_runner = CallbackToolRunner::new(|call: ToolCallEnvelope<TestTools>| async move {
        match call.call {
            TestTools::Ping { value } => Ok(ToolResultEnvelope {

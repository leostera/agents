...
import { MemoryExplorerEdge, createBorgApiClient } from '@borg/api'
import ReactFlow, { Background, Controls, Edge, MarkerType, MiniMap, Node, Panel } from 'reactflow'
...
function extractUriRefs(value: unknown, into: Map<string, Set<string>>, path = '') {
...
function deriveEdgesFromEntities(entities: MemoryEntity[]): MemoryExplorerEdge[] {
...
export function MemoryExplorer({ className, initialQuery = 'borg' }: MemoryExplorerProps) {
  ...
  const graph = useMemo(() => buildGraph(entities, explorerEdges), [entities, explorerEdges])
              ...
              <Panel position='top-left' className='explorer-flow-panel'>
                <div className='explorer-toolbar explorer-toolbar--inside'>
                  ...
                  <input
                    className='explorer-input'
                    value={limit}
                    onChange={(event) => setLimit(Math.max(10, Number(event.currentTarget.value) || 300))}
                    type='number'
                    min={10}
                    max={2000}

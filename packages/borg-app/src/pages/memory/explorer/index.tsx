...
import { Badge, Button, EntityView, Input, Link, Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@borg/ui'
...
export function MemoryExplorerPage({ explorerUri }: MemoryExplorerPageProps) {
  ...
  const [singleResult, setSingleResult] = React.useState<MemoryEntity | null>(null)

  const runSearch = React.useCallback(async (nextQuery: string, syncUrl = true) => {
  ...
  const singleEntity = singleResult ?? (results.length === 1 ? results[0] : null)
            ...
            <TableBody>
              {results.map((entity) => (
                <TableRow
                  ...
                  onClick={() => navigateTo(`/memory/entity/${entity.entity_id}`)}
                >
                  <TableCell className='max-w-56 truncate font-medium'>{entity.label || '—'}</TableCell>
                  <TableCell className='max-w-80 truncate font-mono text-[11px]'>{entity.entity_id}</TableCell>
                  ...
                  <TableCell className='text-right'>
                    <div className='inline-flex gap-1'>
                      ...
                      >
                        <Link href={`/memory/entity/${entity.entity_id}`}>Open</Link>
                      </Button>
                      <Button
                        asChild
                        variant='ghost'
                      ...
                      >
                        <Link href={`/memory/graph?query=${entity.entity_id}`}>Graph</Link>
                      </Button>
                    </div>
                  </TableCell>
                </TableRow>
        ...
        <section>
          {singleEntity ? (
            ...
            <p className='text-muted-foreground text-sm'>
              {query.trim().length === 0 ? 'Search for an entity URI or keyword.' : 'No entities found.'}
            </p>
          )}
        </section>
      )}

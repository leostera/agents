...
function deriveFieldSchema(kindEntities: MemoryEntity[], fieldName: string): FieldSchema {
  ...
  const optionality = values.length === kindEntities.length ? 'required' : `optional (${presenceRatio})`
  const arity =
    listCount === 0 ? 'one' : listCount === values.length ? 'many' : `mixed (${listCount}/${values.length} lists)`
...
export function MemoryShapesPage() {
  ...
  React.useEffect(() => {
    const load = async () => {
      ...
      setError(null)
      try {
        const rows = (await borgApi.searchMemory({ q: '', limit: 500 })) as MemoryEntity[]
        setEntities(rows)
      } catch (loadError) {
        setEntities([])
        setError(loadError instanceof Error ? loadError.message : 'Unable to load shapes')
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const namespaces = React.useMemo(() => Array.from(namespaceMap.keys()).sort((a, b) => a.localeCompare(b)), [namespaceMap])

  React.useEffect(() => {
    if (!selectedNamespace && namespaces.length > 0) setSelectedNamespace(namespaces[0])
  ...
  const kinds = React.useMemo(() => Array.from(kindMap.keys()).sort((a, b) => a.localeCompare(b)), [kindMap])
  ...
  React.useEffect(() => {
    if (!selectedField || !fields.includes(selectedField)) setSelectedField(fields[0] ?? '')
  ...
  return (
    <section className='space-y-4'>
      <p className='text-muted-foreground text-xs'>
        Shapes explorer: click namespace, then kind, then field to inspect schema metadata.
      </p>
      ...
      <div className='grid gap-3 xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2'>
        <Card>
          <CardContent className='p-2'>
            <p className='mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground'>Namespace</p>
              ...
              {namespaces.map((namespace) => (
                <button
                  ...
                  className={`w-full rounded-md border px-2.5 py-2 text-left text-sm ${
                    selectedNamespace === namespace ? 'border-primary bg-muted/40' : 'border-border/60'
                  }`}
                >
                  <div className='flex items-center justify-between gap-2'>
                    <span className='truncate font-medium'>{namespace}</span>
                    <Badge variant='secondary'>{namespaceMap.get(namespace)?.length ?? 0}</Badge>
                  </div>
                </button>
              ))}
            </div>
        ...
        <Card>
          <CardContent className='p-2'>
            <p className='mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground'>Kind</p>
            <div className='max-h-[30rem] space-y-1 overflow-auto'>
              {kinds.map((kind) => (
                <button
                  key={kind}
                  ...
                  className={`w-full rounded-md border px-2.5 py-2 text-left text-sm ${
                    selectedKind === kind ? 'border-primary bg-muted/40' : 'border-border/60'
                  }`}
                >
                  <div className='flex items-center justify-between gap-2'>
                    <span className='truncate font-medium'>{kind}</span>
                    <Badge variant='secondary'>{kindMap.get(kind)?.length ?? 0}</Badge>
                  </div>
                </button>
              ))}
            </div>
        ...
        <Card>
          <CardContent className='p-2'>
            <p className='mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground'>Field</p>
            <div className='max-h-[30rem] space-y-1 overflow-auto'>
              {fields.map((field) => (
                <button
                  key={field}
                  ...
                  className={`w-full rounded-md border px-2.5 py-2 text-left text-sm ${
                    selectedField === field ? 'border-primary bg-muted/40' : 'border-border/60'
                  }`}
                >
        ...
        <Card className='md:col-span-2 lg:col-span-1'>
          <CardContent className='space-y-2 p-3'>
            <p className='text-xs font-semibold uppercase tracking-wide text-muted-foreground'>Field Schema</p>
            {selectedSchema ? (
              <>
                <p className='font-mono text-sm font-medium'>{selectedSchema.name}</p>
                <p className='text-muted-foreground text-xs'>{selectedSchema.description}</p>
                <div className='grid gap-1 text-xs'>
                  <p><span className='text-muted-foreground'>Domain:</span> {selectedSchema.domain}</p>
                  <p><span className='text-muted-foreground'>Range:</span> {selectedSchema.range}</p>
                  <p><span className='text-muted-foreground'>Arity:</span> {selectedSchema.arity}</p>
                  <p><span className='text-muted-foreground'>Optionality:</span> {selectedSchema.optionality}</p>
                </div>
              ...
              <p className='text-muted-foreground text-sm'>Choose a field to inspect schema.</p>
            )}
          </CardContent>
        </Card>
      </div>

...
import { AlertTriangle, Bot, Clock3, DollarSign, ShieldCheck, User } from 'lucide-react'
...
const observabilityData: ObservabilityPoint[] = Array.from({ length: 90 }, (_, index) => {
  const date = new Date(REFERENCE_DATE)
  date.setDate(date.getDate() - (89 - index))

  const inbound = 170 + ((index * 19) % 130) + Math.round(Math.sin(index / 4) * 22)
  const completed = inbound - (8 + (index % 14))
  const p50 = 1.4 + ((index % 10) * 0.05 + Math.abs(Math.sin(index / 7)) * 0.2)
  const p95 = 3.8 + ((index % 9) * 0.1 + Math.abs(Math.cos(index / 8)) * 0.6)
  const errorRate = 1.1 + ((index % 7) * 0.12 + Math.abs(Math.sin(index / 5)) * 0.5)
...
export function OverviewSection() {
  ...
  const throughputDelta = latest && prev ? latest.completed - prev.completed : 0
    ...
    <section className='space-y-4'>
      <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-3'>
        <Card>
          <CardHeader>
            ...
            <CardTitle className='text-xl font-semibold'>`borg:agent:default`</CardTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className='text-xl font-semibold'>`borg:session:telegram_2654566`</CardTitle>
          </CardHeader>
          <CardContent className='space-y-2 text-sm'>
            <div className='flex items-center justify-between'>
            ...
            <div className='flex items-center gap-2 text-xs text-muted-foreground'>
              ...
              6 user messages, 4 tool calls, 1 follow-up
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle className='text-xl font-semibold'>4.5M tokens (24h)</CardTitle>
          </CardHeader>
          <CardContent className='space-y-2'>
            {providerTokenLoad.map((item) => (
              <div key={item.provider} className='flex items-center justify-between rounded-md border px-3 py-2 text-sm'>
                <span>{item.provider}</span>
                <span className='font-medium'>
                  {item.tokens} <span className='text-xs text-muted-foreground'>({item.share})</span>
                </span>
              </div>
            ))}
          </CardContent>
      ...
      <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-5'>
        <Card size='sm'>
          <CardHeader>
            ...
            <CardTitle className='text-2xl font-semibold'>{throughputNow}/h</CardTitle>
          </CardHeader>
        ...
        <Card size='sm'>
          <CardHeader>
            ...
            <CardTitle className='text-2xl font-semibold'>{p95Now.toFixed(1)}s</CardTitle>
          </CardHeader>
          <CardContent className='text-xs text-muted-foreground'>end-to-end</CardContent>
        </Card>
        <Card size='sm'>
          <CardHeader>
            <CardDescription>Error Rate</CardDescription>
            <CardTitle className='text-2xl font-semibold'>{errorNow.toFixed(2)}%</CardTitle>
          </CardHeader>
          <CardContent className='text-xs text-muted-foreground'>provider + runtime</CardContent>
        </Card>
        <Card size='sm'>
          <CardHeader>
          ...
          <CardContent className='text-xs text-muted-foreground'>median wait 3m 42s</CardContent>
        </Card>
        <Card size='sm'>
          <CardHeader>
            <CardDescription>Cost Burn</CardDescription>
            <CardTitle className='text-2xl font-semibold'>${costNow.toFixed(2)}</CardTitle>
          </CardHeader>
      ...
      <div className='grid gap-4 xl:grid-cols-3'>
        <Card className='xl:col-span-2'>
          <CardHeader>
            ...
            <CardDescription>Volume trend by day across platform traffic.</CardDescription>
            <CardAction>
              <div className='flex items-center gap-2'>
                ...
                <Select value={timeRange} onValueChange={(value: RangeKey) => setTimeRange(value)}>
                  <SelectTrigger className='w-24 md:hidden' aria-label='Select chart range'>
          ...
          <CardContent className='pt-2'>
            <ChartContainer config={throughputChartConfig} className='h-72 w-full'>
              <AreaChart data={filteredData} margin={{ left: 8, right: 8 }}>
                <defs>
                  <linearGradient id='fillInbound' x1='0' y1='0' x2='0' y2='1'>
                    ...
                    <stop offset='95%' stopColor='var(--color-inbound)' stopOpacity={0.06} />
                  </linearGradient>
                ...
                />
                <Area dataKey='inbound' type='natural' fill='url(#fillInbound)' stroke='var(--color-inbound)' strokeWidth={2} />
                <Area
                  dataKey='completed'
                  type='natural'
          ...
          <Card>
            ...
            <CardContent className='space-y-2'>
              {incidents.map((incident) => (
                <div key={incident} className='flex items-start gap-2 rounded-md border border-dashed px-3 py-2 text-sm text-muted-foreground'>
      ...
      <div className='grid gap-4 xl:grid-cols-3'>
        <Card className='xl:col-span-2'>
          <CardHeader>
            ...
            <CardDescription>P50 and P95 response latency for completed tasks.</CardDescription>
          </CardHeader>
          <CardContent className='pt-2'>
            <ChartContainer config={latencyChartConfig} className='h-64 w-full'>
              <LineChart data={filteredData} margin={{ left: 8, right: 8 }}>
                ...
                />
                <Line dataKey='p50' type='monotone' stroke='var(--color-p50)' strokeWidth={2.2} dot={false} />
                <Line dataKey='p95' type='monotone' stroke='var(--color-p95)' strokeWidth={2.2} dot={false} />
              </LineChart>
            </ChartContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardDescription>Cost Burn</CardDescription>
            <CardTitle className='text-3xl font-semibold'>${costNow.toFixed(2)}</CardTitle>
            <CardAction>
          ...
          <CardContent className='pt-1'>
            <ChartContainer config={costChartConfig} className='h-44 w-full'>
              <LineChart data={filteredData} margin={{ left: 4, right: 8, top: 6, bottom: 0 }}>
                <CartesianGrid vertical={false} />
                <XAxis
                  dataKey='date'
                  tickLine={false}
                ...
                />
                <Line dataKey='cost' type='monotone' stroke='var(--color-cost)' strokeWidth={2.5} dot={false} />
              </LineChart>
            </ChartContainer>
            ...
            <div className='mt-2 flex items-center justify-between rounded-md border px-3 py-2 text-sm'>
              <span className='text-muted-foreground'>Model retry overhead</span>
      ...
      <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
        <Card size='sm'>
          ...
          <CardContent className='text-xs text-muted-foreground'>Top failures: executeCode, searchApis</CardContent>
        </Card>
        <Card size='sm'>
          <CardHeader>
          ...
          <CardContent className='flex items-center gap-1.5 text-xs text-muted-foreground'>
            ...
            8 entities missing explicit policy
          </CardContent>
        </Card>
        <Card size='sm'>
          <CardHeader>
          ...
          <CardContent className='text-xs text-muted-foreground'>telegram ingress to enqueue</CardContent>
        </Card>
        <Card size='sm'>
          <CardHeader>

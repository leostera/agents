...
5. llama.cpp implementation behind feature flag (`llama-cpp`)
...
`LlamaCppEngine` (feature `llama-cpp`) uses `llama-cpp-2` and `GGUF` files:
...
2. `borg llm test --model <id> --prompt <text> [--gguf <path>]`
   ...
   - prints summary JSON (tokens, duration, finish reason)
...
1. `llama-cpp-2` introduces native build/toolchain requirements (notably clang).
